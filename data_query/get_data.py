...
#Per-frame metadata
write_frames(cf.dr, f'{PATH_SQL}frames_wide.sql', submit=cf.submit, dir_out=cf.paths.data, do_download=cf.download)
...
for fd in cf.get_subfields():
	...
	if nparts > 1:
		...
		for i in range(nparts):
			...
			write_fieldsearch(cf.dr, fd, f'{PATH_SQL}run_{fd.upper()}_part{i+1:02d}.sql', dir_out=cf.paths.data, do_photoz=cf.photoz, mizuki_mstar_sfrs=cf.mizuki_mstar_sfrs, submit=cf.submit, apply_cuts=cf.apply_cuts, strict_cuts=cf.strict_cuts, dec_range=[decmin_now,decmax_now], do_download=cf.download, part=i+1)
	else:
		write_fieldsearch(cf.dr, fd, f'{PATH_SQL}run_{fd.upper()}.sql', dir_out=cf.paths.data, do_photoz=cf.photoz, mizuki_mstar_sfrs=cf.mizuki_mstar_sfrs, submit=cf.submit, apply_cuts=cf.apply_cuts, strict_cuts=cf.strict_cuts, do_download=cf.download)
