...
for fd in cf.fields:
	...
	if nparts > 1:
		...
		for i in range(nparts):
			...
			write_fieldsearch(cf.dr, fd, f'{PATH_SQL}run_{fd.upper()}_part{i+1}.sql', dir_out=cf.PATH_DATA, do_photoz=cf.photoz, submit=cf.submit, strict_cuts=cf.strict_cuts, dec_range=[decmin_now,decmax_now], do_download=cf.download, part=i+1)
	else:
		write_fieldsearch(cf.dr, fd, f'{PATH_SQL}run_{fd.upper()}.sql', dir_out=cf.PATH_DATA, do_photoz=cf.photoz, submit=cf.submit, strict_cuts=cf.strict_cuts, do_download=cf.download)
