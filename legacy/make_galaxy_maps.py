...
def makeNgalMaps(cat, footprint):
    ...
    -------
    ngal_maps: recarray
        recarray for which each entry is a galaxy counts map for a given
        redshift bins.
    '''
    ...
    ngal_maps, _, _ = mu.initialiseRecMap(cf.nside_lo, cf.nside_hi, labels,
                                          pixels=footprint.valid_pixels,
                                          dtypes='f8')

    # Cycle through the sample definitions and count the galaxies in each pixel
    for k in cf.samples:
        sel = cat[k][:]
        _, ngal = mu.countsInPixels(cat['ra'][sel], cat['dec'][sel],
                                    cf.nside_lo, cf.nside_hi,
                                    footprint.valid_pixels,
                                    return_vals=True)
...
def makeDensityMaps(ngal_maps, mask):
    '''
    Uses galaxy counts maps along with a mask to create maps of delta_g
    (galaxy overdensity) in each tomographic bin. Assumes one mask for all
    bins.

    Parameters
    ----------
    ngal_maps: recarray
        recarray for which each entry is a galaxy counts map in each of the
        specified bands.

    mask: MaskData object
        Survey mask, in which each pixel is assumed to effectively be the
        detection fraction.

    Returns
    -------
    deltag_maps: recarray
        recarray for which each entry is a galaxy overdensity map for a given
        redshift bin.
    '''

    print('Creating delta_g maps...')
    # Identify pixels in the mask above the specified threshold
    # NOTE: due to the way in which the maps and masks are constructed, they
    # should all have the same valid_pixels

    # Initialise a recarray to contain the maps for each band
    labels = [k for k in cf.samples]
    # Make a copy of the inputted recarray
    deltag_maps, *_ = mu.initialiseRecMap(cf.nside_lo, cf.nside_hi, labels,
                                          pixels=mask.vpix_nest, dtypes='f8')
    for k in cf.samples:
        # Calculate mean galaxy counts and mean weight for pixels being kept
        ...
        deltag_maps[k][mask.vpix_nest] = (ngal_maps[k][mask.vpix_nest]
                                          / (mask.mask_hsp[mask.vpix_nest]
                                             * mu_n / mu_w)) - 1.
...
for fd in cf.fields:
    ...
    # Make galaxy count maps in each redsift bin and store in a single recarray
