###############################################################################
# - Estimates the n(z) distributions for each tomographic bin using the direct
#   reweighted estimation method (DIR). Uses many-band photo-zs from COSMOS as
#   the 'true' valus for calibration.
# - NOTE: Unlike other stages, this script has no dependence on the HSC field
#   being analysed, as the n(z) distributions are computed using specific
#   catalogues.
# - TODO:
#   - Adapt script so that the data can be automatically retrieved if they do
#     not exist?
###############################################################################
...
if os.path.exists(cf.hsc_cosmos_cat):
    ...
    with h5py.File(cf.hsc_cosmos_cat, 'r') as hf:
        ...
        cols_hsc += [f'pz_{s}_dnnz' for s in ['mean', 'mode', 'best', 'mc',
                                              'err68_min', 'err68_max',
                                              'err95_min', 'err95_max']]
    ...
    raise FileNotFoundError(
        f'File {cf.hsc_cosmos_cat} not found. '
        'Download the HSC data using the SQL query in '
        'data_query/run_COSMOS_DUD.sql then use the cleanCatalogues stage of '
        'the pipeline to create a cleaned version. '
        'hsc_cosmos_cat should then be set to the path of this cleaned '
        'version.'
    )
...
if os.path.exists(cf.cosmos_cat):
    ...
    raise FileNotFoundError(
        ...
        "'wget https://cdsarc.cds.unistra.fr/ftp/J/ApJS/258/11/fits/"
        "COSMOS2020_CLASSIC_R1_v2.1_p3.fits'. "
        'cosmos_cat should then be set to the path of this file.'
    )

# Apply cuts to the COSMOS data
cosmos_mask = (cosmos_data['lp_zMinChi2'] > 0.01) & \
            (cosmos_data['lp_zMinChi2'] < 9) & \
            (cosmos_data['lp_type'] == 0.) & \
            (cosmos_data['lp_mass_best'] > 7.5) & \
            (np.maximum(cosmos_data['lp_zPDF_u68'] - cosmos_data['lp_zPDF'],
                        cosmos_data['lp_zPDF'] - cosmos_data['lp_zPDF_l68'])
                < 0.05 * (1. + cosmos_data['lp_zMinChi2'])) & \
...
cosmos_skycoord = SkyCoord(cosmos_data['ALPHA_J2000'],
                           cosmos_data['DELTA_J2000'])
...
##########################################
train_sample = np.array([np.array(cat_matched[f'{b}_cmodel_mag'])
...
n_nbrs = NearestNeighbors(n_neighbors=cf.kNN,
                          algorithm='kd_tree',
                          metric='euclidean'
                          ).fit(train_sample)
...
# Find all sources in the full HSC data within the distance to the 20th
# nearest neighbour
...
# Compute weights as the ratio of the number of photo-z neighbours
# (N_HSC=num_photoz) to the number of COSMOS neighbours (N_COSMOS=kNN),
# normalised by the number of photo-z (HSC) objects
...
with h5py.File(cf.paths.out + cf.nofz_files.nz_dir, 'w') as hf:
    ...
    for zb in zbin_masks:
        nz, _ = np.histogram(cat_matched['lp_zMinChi2'][zbin_masks[zb]],
                             bins,
                             weights=weights[zbin_masks[zb]],
                             density=True)
