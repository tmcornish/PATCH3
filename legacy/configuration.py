...
class PipelineConfig():
    ...
    def _set_output_names(self):
        ...
        for key in self.maps:
            ...
            if key == 'dustmaps':
                self.config_dict['maps'][key] = [self.maps[key] +
                                                 f'_{b}_nside{self.nside_hi}' +
                                                 f'{self.suffix}.hsp'
                                                 for b in self.bands.all]
            else:
                self.config_dict['maps'][key] = self.maps[key] + \
                    f'_nside{self.nside_hi}{self.suffix}.hsp'

        # Data files
        for key in self.data_files:
            self.config_dict['data_files'][key] = self.paths.data + \
                self.data_files[key] + '.fits'

        # n(z) hdf5 files
        for key in self.nofz_files:
            self.config_dict['nofz_files'][key] = self.nofz_files[key] + \
                self.suffix + '.hdf5'

        # Power spectra hdf5 files
        for key in self.cell_files:
            self.config_dict['cell_files'][key] = self.cell_files[key] + \
                f'_nside{self.nside_hi}{self.suffix}.hdf5'

        # Cache files
        for key in self.cache_files.workspaces:
            self.config_dict['cache_files']['workspaces'][key] = \
                self.cache_files.workspaces[key] + \
        ...
        for key in self.sacc_files:
            self.config_dict['sacc_files'][key] = \
                self.sacc_files[key] + \
                f'_nside{self.nside_hi}{self.suffix}.fits'

        # Auxiliary files (assumes stored in same directory as config file)
    ...
    def _set_flags(self):
        '''
        Retrieves the column names for all flags to be used when excluding
        sources from analysis, using names listed in the file specified as
        self.auxfiles.flags.
        '''
        ...
        # Brightstar flags are assumed to be associated with the primary band
        # unless they begin with the band name
        self.config_dict['flags']['brightstar'] = [f'{self.bands.primary}{fl}'
                                                   if fl.startswith('_')
                                                   else fl
                                                   for fl
                                                   in flags_dict['brightstar']]

        # 'main' flags are associated with the primary band
        self.config_dict['flags']['main'] = [f'{self.bands.primary}{fl}'
                                             for fl in flags_dict['main']]
    ...
    def get_samples(self, cat):
        '''
        Returns masks to apply to the input catalogue in order to select
        sources belonging to each sample.
        ...
        sample_masks: dict[numpy.array]
            Dictionary of boolean arrays identifying which sources in the
            catalogue belong to each sample.
        '''
        import numpy as np
        ...
        for s in self.samples:
            ...
            samp_mask = np.ones_like(cat[next(iter(cat.keys()))][:],
                                     dtype=bool)
            for ex in samp:
                samp_mask *= eval(ex)
    ...
    def combine_flags(dataset, flagcols, combine_type='or'):
        ...
        if combine_type not in ['or', 'and']:
            raise TypeError('combine_type must be either "and" or "or" '
